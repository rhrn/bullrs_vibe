...
impl Worker {
    ...
    async fn get_next_job(
        ...
        // Specify return type for Redis commands to address future never type issue
    ...
    async fn process_job(
        ...
        // Simple job processing implementation
        // Process the job
        ...
        match result {
            Ok(return_value) => {
                ...
                redis::cmd("EVAL")
                    ...
                    .arg(job_data)
                    .arg("completed")
                    .arg(job.options.remove_on_complete.unwrap_or(false).to_string())
                    .query(&mut conn)
                    .map_err(|e| BullRsError::RedisError(e))?;
            }
            Err(err) => {
                ...
                redis::cmd("EVAL")
                    ...
                    .arg(job_data)
                    .arg("failed")
                    .arg(job.options.remove_on_fail.unwrap_or(false).to_string())
                    .query(&mut conn)
                    .map_err(|e| BullRsError::RedisError(e))?;
                ...
                if job.can_retry() {
                    ...
                    let delay = match &job.options.backoff {
